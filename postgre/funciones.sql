...
--LISTO
select * from checkstock(1676326,1,96792430,7101,'18/11/2018','19/11/2018');
...
SELECT * FROM SUCURSAL_HERRAMIENTA SU NATURAL JOIN HERRAMIENTA H
WHERE SU.DESCUENTO IS NOT NULL;

SELECT * FROM VERIFICAR_DESCUENTO(3087158,96792430,1);
CREATE OR REPLACE FUNCTION VERIFICAR_DESCUENTO(
    CODIGO_H IN SUCURSAL_HERRAMIENTA.COD_HERRAMIENTA%TYPE,
    CODIGO_E IN SUCURSAL_HERRAMIENTA.EMPRESA%TYPE,
    CODIGO_S IN SUCURSAL_HERRAMIENTA.COD_SUCURSAL%TYPE)
    RETURNS INT AS $$
DECLARE
    DESCUENTO_H INT;
...
SELECT * FROM ISNUMERIC(1);

CREATE OR REPLACE FUNCTION ISNUMERIC(valor in int) 
RETURNS BOOLEAN AS $$
DECLARE 
    RETORNO BOOLEAN;
BEGIN
